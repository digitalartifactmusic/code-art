...
static std::vector<std::thread> THREADS(USED_THREADS);
...
void iterateMandelbrot(const unsigned thread)
{
	...
	while (i < SIZE)
	{
		...
		while (1)
		{
			...
			if (((k.real() * k.real()) + (k.imag() * k.imag())) > 4.0)
			{
	...
	switch (JULIA)
	{
		case false:
		{
			...
			while (WINDOW.isOpen())
			{
				...
				for (j = 0; j < USED_THREADS; j++)
					THREADS[j] = std::thread(iterateMandelbrot, j);
				for (auto& j : THREADS)
					j.join();
		...
		case true:
		{
			...
			while (WINDOW.isOpen())
			{
				...
				for (j = 0; j < USED_THREADS; j++)
					THREADS[j] = std::thread(iterateJulia, j);
				for (auto& j : THREADS)
					j.join();

				WINDOW.draw(VERTEXARRAY);
				WINDOW.display();
				JULIAPOINT *= std::complex<long double>{ cos(3.1415926 / 50000.0), sin(3.1415926 / 50000.0) };
				JULIAPOINT += std::complex<long double>{ -0.000001L, -0.000001 };
			}
			break;
		}
	}
